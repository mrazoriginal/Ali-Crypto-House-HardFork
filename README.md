# Ali Crypto House – Backend Edition 💻🚀
...
This fork adds a **Node.js backend** for API endpoints and portfolio storage while keeping all frontend features ✨ intact.
...
- **Particle Fix:** Solved that annoying bug where the floating particles overlapped with the boxes. Now everything stays crisp and clean.  
- **Temporary Storage:** Your input values are now saved in my cloud (or local storage as backup), so your data persists after a page refresh.  
...
- **Live Coin Prices:** Bitcoin (BTC), Ethereum (ETH), and Tether (USDT) updated every 2 seconds via API (or server proxy).  
...
- **Node.js + Express** – Simple backend server for API endpoints and portfolio persistence.  
